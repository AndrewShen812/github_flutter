...
import 'package:github_flutter/common/utils.dart';
import 'package:github_flutter/i10n/localization_intl.dart';
import 'package:github_flutter/models/index.dart';
...
class _LoginRouteState extends State<LoginRoute> {
  ...
  Widget build(BuildContext context) {
    var gm = GhLocalizations.of(context);
    return Scaffold(
  ...
  void _onLogin() async {
    ...
    if ((_formKey.currentState as FormState).validate()) {
      showLoading(context, text: '登录中，请稍候...');
      User user;
      try {
        user = await GithubApi(context).login(_unameController.text, _pwdController.text);
        ...
        if (e.response?.statusCode == 401) {
          showToast(context, GhLocalizations.of(context).userNameOrPasswordWrong);
        } else {
